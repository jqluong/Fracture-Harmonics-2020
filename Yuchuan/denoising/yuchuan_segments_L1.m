...
%weights
mu = 0.03;  %weight for the ||f-g||^2 term
lambda = 1;   %weight for the ||f'||^2 term
eta = 100000; %weight for the continuity term
...
ineq = sparse([[P;sparse(1,2*n)] -1*speye(n) ; -1*[P;sparse(1,2*n)] -1*speye(n) ; D -1*speye(n) ; -1*D -1*speye(n)]);


%% Minimization
options = optimoptions(@quadprog, 'Algorithm', 'interior-point-convex', 'OptimalityTolerance', 1e-8);
ft = quadprog(2*H,v,ineq,zeros(4*n,1));
f = ft(1:2*n);

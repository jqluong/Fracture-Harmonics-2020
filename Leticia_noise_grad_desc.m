...
for j = 1:iterations
    ...
    % old idea for loop and its update ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    % create grad_H2
    %for k = 1:n
    %    if grad_h(k) > 0
    %        grad_H2(k) = 1;
    ...
    %end
    % update h
    %h = h_old - tau*( (h_old-g) + lamb*( grad_H2 ) );
    %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    
    % initial idea of h update: ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    %arg_H2 = abs(grad_h) / sqrt( e^2 + ( norm( grad_h) )^2 );
    
    %del_H2 = D*transpose(arg_H2);
  
    % the h update: 
    % h = h_old - tau_2*( (h_old-g) - lamb_2*( transpose(del_H2) ) ) ;
    %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

function face_min_dirichlet_L1_bc(V,F, B, num, filename)
...
Y = eigenmodes_iterations(Vd,Fd,D,m,k,B, Y,num);

% plot eigenmode functions
animated_eigenmodes_bc(Vd,Fd,k, Y, filename)

function  R = eigenmodes_iterations(Vd,Fd,D,m,k,B, Y,num)
    ...
    % contruct boundary condition equality matrix and vector
    [s,~] =  size(B);
    ...
    AeqB = sparse(Aeq_i,AeqB_j,AeqB_v,s,3*k);
    ...
    function perform_iter(i)
        ...
        beq = [ zeros((i-1),1); 1; transpose(g)];           % inequality constraint vector
        ...
        while sqrt(transpose(c-Y(:,i))*M*(c-Y(:,i)))  > CONVERG
            ...
            Aeq = [ Eq_0 zeros(length(Eq_0(:,1)),2*m); Eq_1 zeros(length(Eq_1(:,1)),2*m); AeqB zeros(length(AeqB(:,1)),2*m)];
...
function animated_eigenmodes_bc(V,F,k, Y, filename)

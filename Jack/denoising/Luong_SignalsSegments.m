...
hold off
title(['Denoising Algorithm with Line Segment Functions with parameter ', num2str(lambda)])
legend([p1 p4])

%Plot Gradients of Denosied Functions
figure
hold on
plot(1:stepsize, (computeDerivative(stepsize)*l2Solution.recoveredSignal)', 'LineWidth', 1)
plot(1:stepsize, (computeDerivative(stepsize)*l1Solution.recoveredSignal)', 'LineWidth', 1)
...
%solution
function l2Solution = SolveL2Problem(noisyFunction, stepsize, weight)
...
%solution
function l1Solution = SolveL1Problem(noisyFunction, stepsize, weight)
    noisyFunctionReg = zeros(2*stepsize, 1);
    for i = 1:stepsize
        noisyFunctionReg(i) = noisyFunction(i,1);
        noisyFunctionReg(stepsize + i) = noisyFunction(i,2); 

...
[f history] = total_variation(s, lambda, 1.0, 1.0);
...
%% ADMM function
% taken from: https://web.stanford.edu/~boyd/papers/admm/
...
% derivative
Dm = discrete_derivative(n,1/(n-1));
...
for k = 1:MAX_ITER
    ...
    history.objval(k)  = objective(b, lambda, Dm, x, z);
...
function obj = objective(b, lambda, D, x, z)
    % obj = .5*norm(x - b)^2; + lambda*norm(z,1); % z is 2 point forward difference derivative
    obj = .5*norm(x - b)^2; + lambda*norm(D*x,1); % D*x is 3 point difference derivative
end
